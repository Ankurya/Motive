...
<?php //echo $user_list->public_interest; 
// echo '<pre>';
// print_r($user_list->age); exit; ?>
<?php // if(strpos($user_list->public_interest,'2') !== false) echo "hello"; ?>
	...
	@if($errors->any())
				...
				<p class="alert alert-success" style="color:black;">{{ Session::get('status') }}</p>
					@endif
		<div class="container-fluid">
			<form action="{{url('update_user')}}" method="post" class="main-content-form" enctype="multipart/form-data">
				{{ csrf_field() }}
				...
				<div class="form-group col-sm-6 col-xs-12 ">
				<label>Age</label>
					<input type="text" name="age" class="form-control border-input" value="@if(!empty($user_list->age)) {{$user_list->age}}@endif">
				</div>
